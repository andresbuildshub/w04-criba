...
export default async function Tablero() {
  ...
  const estado = s.cumple ? 'CUMPLIDA: el piloto sigue' : s.cerrado ? 'NO CUMPLIDA: el piloto se detiene y cambia de problema' : 'PENDIENTE: faltan respuestas'
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <p className="text-sm uppercase tracking-widest text-amber-300">Regla del piloto: 5 de 20</p>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className={`rounded-2xl border p-5 ${s.cumple ? 'border-emerald-400/40 bg-emerald-400/5' : s.cerrado ? 'border-rose-400/40 bg-rose-400/5' : 'border-amber-300/40 bg-amber-300/5'}`}><div className="text-xs uppercase text-neutral-500">Regla 5 de 20</div><div className="mt-1 font-semibold">{estado}</div></div>
      </section>
      ...
      <p className="text-xs text-neutral-500">Las respuestas se guardan en el servidor sin ningún dato personal{persistencia.startsWith('memoria') ? ' (modo de prueba: no persistente)' : ''}.</p>
    </div>
  )
}
